...
def fit_budget_shock(x1, state, debt, debtchoice, choices, ccp_path, budget, terminal_data, period):
    ...
    moments = get_moments_by_x1(x1, debtchoice, nmoments=4).astype(np.float64)

    # ---- initial guess
    params0 = np.array([
        5000.0, 5000.0, 5000.0, 5000.0,   # mu1..mu4
        100.0,                            # sigma_e
        2.0, 2.0, 2.0, 2.0,               # ra1..ra4
        -2.0, -2.0, -2.0, -2.0            # debtpen1..debtpen4
    ], dtype=np.float64)
    ...
    bounds = [
        (MU_MIN, MU_MAX), (MU_MIN, MU_MAX), (MU_MIN, MU_MAX), (MU_MIN, MU_MAX),  # mu1..mu4
        (SIGE_MIN, SIGE_MAX),                                                    # sigma_e
        (0.1001, 2.9999), (0.1001, 2.9999), (0.1001, 2.9999), (0.1001, 2.9999),  # ra
        (-1e6, 0.0), (-1e6, 0.0), (-1e6, 0.0), (-1e6, 0.0)                       # debtpen
    ]
...
def solve_one_draw_debt_idx(
    ...
    """
    Solve optimal debt index for ONE draw of e (n,).

    Fallback rule:
      If for an individual i *no* debt choice j in [b_idx[i], max_idx[i]]
      yields c >= c_floor, then set choice to fallback_idx (default 99).

    Notes:
      - fallback_idx should be a valid index into debt_grid (e.g., 99 for size 100).
      - If your grid size ever changes, pass fallback_idx=debt_grid.size-1.
    """
    n = budget.shape[0]
    B = debt_grid.size
    out = np.empty(n, dtype=np.int64)

    # make sure fallback is in range
    fb = fallback_idx
    if fb < 0:
        fb = 0
    if fb >= B:
    ...
    for i in prange(n):
        ...
        # Fallback: if nothing satisfied c>=floor, force max debt index (99)
        # If nothing satisfied c>=floor, force legal maximum debt (cap-consistent)
...
def minimize_distance(params, moments, budget, terminal_data, previousdebt, ccp_path,
    ...
    ra_levels   = params[5:9]
    debt_levels = params[9:13]

    # ---- groups (cheap to recompute; or pass them in)
    g, gi, levels = parinc_groups(x1, col=0)  # levels like [1,2,3,4], gi in 0..3

    # ---- per-individual parameter vectors (vectorized)
    mu_i      = mu_levels[gi].astype(np.float64)
    sigma_i   = ra_levels[gi].astype(np.float64)
    debtpen_i = debt_levels[gi].astype(np.float64)

    # ---- terminal (depends on risk aversion -> ra_levels)
    n = budget.shape[0]
    B = debt_range.size
    ...
    cache = {}  # one cache per evaluation
    ...
    for k in range(s):
        # shock draw for this k (depends on params)
        e = mu_i + sigma_e * Z[k, :]

        # solve optimal debt indices for this draw using bounds
        ...
        simumoments[:, k] = get_moments_by_x1(x1, debtvalue, nmoments=4)
