...
pathfunctions   = DIR["MODEL_FUNCOEF"]     # where wage_*, sigmas.npy, etc live
path_realdata   = DIR["MODEL_REALDATA"]    # where real_data/*.npy live
path_estimates  = DIR["MODEL_ESTIMATES"]   # estimates/*.npy (if needed)

# helpers that yield full paths
def STATES(period: int) -> Path:         return Path(OUT("states", str(period)))
def VJT_NODEBT(period: int) -> Path:     return Path(OUT("vjt_nodebt", str(period)))
def EVT_NODEBT(period: int) -> Path:     return Path(OUT("evt_nodebt", str(period)))
def CONTINUATION() -> Path:              return Path(OUT("continuation"))
#-----------------------------------------------------------------------------#
# Curent model is two periods. Choices are: 
    #-> educ 2 year with debt
    #-> educ 4 year with debt
    #-> work    
...
#@njit()
def get_utility(sigma_u,x1,x1_new,x2,b,b1,e,j,period):
...
def VT(x1,x1_new,x2,x2_new,b,period,j,evt,repayment):
    ...
    if ((x2[1] >=1) & (j[1] == 1) & (x2[4] == 0)) | ((x2[2]>=3) & (j[1] == 2) & (x2[5] == 0))  | (j[1] == 3) :
        ...
        evt_grad =  evt[f"evt_t{period+1}_{x1}_{grad_x2}"][0]
        evt_nograd = evt[f"evt_t{period+1}_{x1}_{notgrad_x2}"][0]
        ...
        while check < 1:
            ...
            # Analyze the case where it is not equal to 0 
                ...
                if c2[bound_left] < 0:
                    ...
                    c2new = c2temp[c2temp>0]
                    # Notice that if there is no posible positive value of consumption
                    # I will set debt to the current value.
                    if len(c2new) == 0: 

                        vjt = get_power_utility(sigma_u,np.maximum(c2[np.maximum(it,maxdebt)],2000)) + continuation[np.maximum(it,maxdebt)]
                        
                        payoff[it+np.shape(continuation)[0]*e] = vjt
                ...
                # Compute th==e maximum and the argmax
                ...
                # Now debt will just grow with interest rates. Set same for simplicity
                
                payoff[it+np.shape(continuation)[0]*e] = get_power_utility(sigma_u,np.maximum(c2[it],2000)) + continuation[it]
...
def get_conditional(sigma_u,x1,x1_new,x2,x2_new,b,b1,e,j,period,evt,conterfactual,maxdebt):
    
    "this function returns the conditional value function associated with each alternative"
    " At some point I will have to adjust this function for periods! Right now is basic. "
        
    
    # Notice that in the VT x2 new is a necessary input but it does not matter 
    # when the individual is at t < T.
    
    # Identify wether debt is being chosen or followed using a repayment scheme 
  
        
    c = get_utility(sigma_u,x1,x1_new,x2_new,b,b1,e,j,period)
    continuation = beta*VT(x1,x1_new,x2,x2_new,b1,period,j,evt,0)
    
    c[c<2000] = 2000

    u = get_power_utility(sigma_u,c) + continuation  
...
def get_expected_conditional(sigma_u,x1,x1_new,x2,x2_new,b,b1,j,period,deg,evt,conterfactual,maxdebt):
   
   '''This function computes a gaussian quadrature expectation of the conditional
   value function. The method is Gauss-Hermite quadrature for indefinite integrals
   ''' 
   # If it is the terminal period, expected conditional is terminal value: 

   e,w = get_quadrature(deg,mu,sigmas)
   # get the vjt for each e
   vjte = get_conditional(sigma_u,x1,x1_new,x2,x2_new,b,b1,e,j,period,evt,conterfactual,maxdebt)
   vjt = vjte*w
   #vjte = vjte.reshape((deg,np.shape(b)[0])).T

          
   # Compute the average across columns
   #np.save(f"vjte/vjte_{j}_{x1}",vjte)  
   vjt = np.sum(vjt, axis=1)
...
def get_all_choices(x1,x1_new,x2,x2_new,b,b1,period,evt,ccp_real,sigma_u,utility_parameters,models,solution_mode,conterfactual,maxdebt):
    ...
    deg = 5  # degree of the quadrature
    
    if conterfactual == 0:
        Jx  = get_possible_choices_nodebt(x1_new,x2,deg)
    elif conterfactual == 1:
        Jx = get_possible_choices(x2)
    # now loop over all the entrances in Jx
    # Create a matrix that will store the values
    
    all_vjt = np.zeros((1,np.shape(Jx)[0]))
    ...
    for j in Jx:
        ...
        # get the choice
        #tic = time.time()
        vjt = get_expected_conditional(sigma_u,x1,x1_new,x2,x2_new,b,b1,j,period,deg,evt,conterfactual,maxdebt)
        #toc = time.time()
    ...
    if solution_mode == 0:
        ...
        if ccp_real == 0:
            ...
            ccp = np.exp(all_vjt[:,base]) / np.sum(np.exp(all_vjt),axis=1)
        else:
...
def get_terminal_pandas(evt,x1,x2,sigma_u,conterfactual):
    ...
    evt = evt[f"con_last{lastschool}_educ{educ}_major{major}"][0]
...
def loop_rows(i,x1,x2,b,b1,period,evt,ccp_real,sigma_u,utility_parameters,models,solution_mode,conterfactual,maxdebt):
    ...
    x2 = np.array(x2,dtype="int")
    if period == T:
        #debt_trajectory = np.load("debt_trajectories/trajectories.npz")
        #vt = terminal_value(debt_trajectory, inv, x2, b, sigma_u)
        vt = get_terminal_pandas(evt,x1,x2,sigma_u,conterfactual)[...,None]
        return vt
    elif period != 1:
        all_vjt, exp_vjt  = get_all_choices(inv,x1_new,x2,x2_new,b,b1,period,evt,ccp_real,sigma_u,utility_parameters,models,solution_mode,conterfactual,maxdebt)
        #get_expected_continuation(inv,x1_new,x2,x2_new,b1,period)
        return all_vjt, exp_vjt
    else:
        all_vjt, exp_vjt  = get_all_choices(inv,x1_new,x2,x2_new,b,b1,period,evt,ccp_real,sigma_u,utility_parameters,models,solution_mode,conterfactual,maxdebt)
        return all_vjt, exp_vjt
...
def loop_over_states(i, x1, x2_set, b, b1, period, ccp_real, sigma_u, utility_parameters,models,solution_mode,conterfactual,evtnew,em_type,maxdebt,save_evt = 1):
    ...
    if period < T:
        ...
        if conterfactual == 1 :
            evt = np.load(f"continuation/continuation_conter_s{x1i[0,2]}_eth{x1i[0,3]}_sigma{sigma_u}.npz")
        else:
            evt = np.load(f"continuation/continuation_s{x1i[0,2]}_eth{x1i[0,3]}_sigma{sigma_u}.npz")
        
    for x2 in x2_set:
        print(i,x2)
        tic = time.time()
        if period < T:
            all_vjt, exp_vjt = loop_rows(i, x1, x2, b, b1, period,evt, ccp_real, sigma_u, utility_parameters,models,solution_mode,conterfactual,maxdebt)
    ...
    # Store things
    if period < T: 
        # Now save all the arrays together
        if solution_mode == 0: 
            pass
        elif solution_mode == 1:
            np.savez(f"vjt_nodebt/{period}/vjt_t{period}_{x1i}_em{em_type}_conter{conterfactual}.npz",**{name:value for name,value in zip(names_vjt,result_vjt)})
            np.savez_compressed(f"evt_nodebt/{period}/evt_t{period}_{x1i}_em{em_type}_conter{conterfactual}.npz",**{name:value for name,value in zip(names_exp,result_exp)})

    else:
        if save_evt == 1:
            if solution_mode == 0:
                pass
                #np.savez_compressed(f"evt_nog/evt_t{period}_{x1i}.npz",**{name:value for name,value in zip(names_exp,result_exp)})
            elif solution_mode == 1:
                np.savez_compressed(f"evt_nodebt/{period}/evt_t{period}_{x1i}_em{em_type}_conter{conterfactual}.npz",**{name:value for name,value in zip(names_exp,result_exp)})
...
def get_all_evt(i,x1,b,b1,ccp_real,sigma_u,utility_parameters,models,solution_mode,conterfactual,em_type,maxdebt):
    ...
    for period in range(T,0,-1):
        ...
        evtnext = loop_over_states(i, x1, x2_set, b, b1, period, ccp_real,sigma_u,utility_parameters,models,solution_mode,conterfactual,evt,em_type,maxdebt)
...
def get_data(period,real_data=1):
    ...
    if real_data == 0 :
        ...
        state   = np.load(f"{path_realdata}/state_t{period}.npy")
        choices = np.load(f"{path_realdata}/choice_t{period}.npy")
        x1      = np.load(f"{path_realdata}/invariant_state_t{period}.npy")[:,1:]
        debt    = np.load(f"{path_realdata}/financial_t{period}.npy")[:,0]
...
def get_feasible():
    ...
    for period in range(1,T):
        ...
        np.save(f"{path_realdata}/state_feasible_t{period}.npy", state_feasible)
        np.save(f"{path_realdata}/invariant_state_feasible_t{period}.npy", x1_feasible)
        np.save(f"{path_realdata}/debt_feasible_t{period}.npy", debt_feasible)
        np.save(f"{path_realdata}/choice_feasible_t{period}.npy", choices_feasible)
...
def get_data_feasible(period):
    ...
    state   = np.load(f"{path_realdata}/state_feasible_t{period}.npy")
    choices = np.load(f"{path_realdata}/choice_feasible_t{period}.npy")
    x1      = np.load(f"{path_realdata}/invariant_state_feasible_t{period}.npy")
    debt    = np.load(f"{path_realdata}/debt_feasible_t{period}.npy")
...
#maxdebt = True
#utility_parameters = load_param_g(em_type,real=0)
#get_all_evt(32,invariant_states,debt_range,debt_range,ccp_real,sigma_u,utility_parameters,models,solution_mode,conterfactual,em_type,maxdebt)
