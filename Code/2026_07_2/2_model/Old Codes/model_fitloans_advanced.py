...
def fit_budget_shock(x1, state, debt, debtchoice, choices, ccp_path, budget, terminal_data):
    ...
    params0 = np.array([
        -12000.0, -12000.0, -12000.0, -12000.0,  # mu1..mu4
        8000.0,                                   # sigma_e
        2.0, 2.0, 2.0, 2.0,                        # ra1..ra4 in [0.1,3]
        -2000.0, -2000.0, -2000.0, -2000.0         # debtpen1..debtpen4 (should be <= 0 if “dislike”)
    ])
    ...
    while newerror > tolr:
        ...
        res = minimize(
            minimize_distance,
            params0,
            args=(moments, budgetsample, terminal_sample, debtsample, evt_ccp, x1sample, statesample, choicessample),
...
def get_debt_rules(c,vjt,previousdebt,state, choice):
    
    """
    This function imposes limits on the feasible debt decisions, either because 
    individuals need some minimum consumption but also because there are limits
    on the student loans. 
    
    Currently, I will enforce a maximum per-period loans, and also a maximum,
    total amount of loans. I will enforce the independent status limits both
    for undergrad and grad individuals. 
...
def get_optimal_debt_by_group(x1, budget, evt, e, previousdebt, g,
                              sigma_risk_levels, debt_penalty_levels, state, choice):
    global debt_range
...
def minimize_distance(params, moments, budget, terminal_data, previousdebt, ccp_path, x1, state, choice):
    """
    params = [
        mu1, mu2, mu3, mu4,         # 0:4
        sigma_e,                    # 4
    ...
    # individual-specific mean for normal draw
    mu_i = mu_levels[gi]
